...
/**
 * Load AiTrend Satistics
 *
 * @returns {undefined}
 */
function loadAiTrendStatistics(version, stockId, dateFrom, dateTo) {
	var seriesCounter = 0, date_price = [], volume = [], data_lonTerm = [], data_midTerm = [], data_shoTerm = [], date_topArea = [], date_bottomArea = [], data_bottomGordon = [];
	...
	$.getJSON(url_price, function(data) {
		...
		seriesCounter += 1;
		if (seriesCounter === 2) {
			createChart_AiTrend_Statistics(stockId, date_price, volume,
					data_lonTerm, data_midTerm, data_shoTerm, date_topArea,
					date_bottomArea, data_bottomGordon);
		}
	});
	 ...
	 * @returns {undefined}
	 */
	var url_ind = getEasyStoGuServerUrl() + "/portal/statistics/aitrend/"
			+ dateFrom + "_" + dateTo;
	$.getJSON(url_ind, function(data) {
		i = 0;
		for (i; i < data.length; i += 1) {
