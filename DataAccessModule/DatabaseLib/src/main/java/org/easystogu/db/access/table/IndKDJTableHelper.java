...
public class IndKDJTableHelper{
	...
	public List<KDJVO> getByIdAndBetweenDate(String stockId, String StartDate, String endDate) {
		try {
			MapSqlParameterSource namedParameters = new MapSqlParameterSource();
			namedParameters.addValue("stockId", stockId);
			...
			List<KDJVO> list = this.namedParameterJdbcTemplate.query(QUERY_BY_STOCKID_AND_BETWEEN_DATE, namedParameters,
					new KDJVOMapper());
			return list;
		} catch (Exception e) {
			e.printStackTrace();
