...
public class IndShenXianTableHelper{
	...
	public List<ShenXianVO> getByDate(String date) {
		try {
			...
			List<ShenXianVO> list = this.namedParameterJdbcTemplate.query(QUERY_BY_DATE_SQL, namedParameters,
	...
	public List<ShenXianVO> getByIdAndBetweenDate(String stockId, String StartDate, String endDate) {
		try {
			MapSqlParameterSource namedParameters = new MapSqlParameterSource();
			namedParameters.addValue("stockId", stockId);
			...
			List<ShenXianVO> list = this.namedParameterJdbcTemplate.query(QUERY_BY_STOCKID_AND_BETWEEN_DATE,
					namedParameters, new ShenXianVOMapper());
			return list;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public List<ShenXianVO> queryByStockId(String stockId) {
